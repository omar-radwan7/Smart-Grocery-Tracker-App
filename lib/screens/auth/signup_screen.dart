...
class _SignupScreenState extends State<SignupScreen>
  ...
  Future<void> _signup() async {
    ...
    if (!ok && mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(auth.error ?? s.get('signupFailed')),
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(s.get('createAccount'),
                      style: const TextStyle(fontSize: 26, fontWeight: FontWeight.w800, color: AppTheme.textDark)),
                  const SizedBox(height: 6),
                  Text(s.get('signupSubtitle'),
                      style: const TextStyle(fontSize: 14, color: AppTheme.textLight)),
                  const SizedBox(height: 28),

                  _label(s.get('fullName')), const SizedBox(height: 8),
                  TextFormField(
                    controller: _nameCtrl,
                    decoration: InputDecoration(
                      hintText: s.get('nameHint'),
                      prefixIcon: const Icon(Icons.person_outline, size: 20, color: AppTheme.textHint),
                    ),
                    validator: (v) => v == null || v.isEmpty ? s.get('enterName') : null,
                  ),
                  const SizedBox(height: 16),

                  _label(s.get('email')), const SizedBox(height: 8),
                  TextFormField(
                  ...
                  _label(s.get('password')), const SizedBox(height: 8),
                  TextFormField(
                    ...
                    validator: (v) {
                      if (v == null || v.isEmpty) return s.get('enterAPassword');
                  ...
                  _label(s.get('confirmPassword')), const SizedBox(height: 8),
                  TextFormField(
                    controller: _confirmCtrl,
                    obscureText: true,
                    decoration: const InputDecoration(
                      hintText: '••••••••',
                      prefixIcon: Icon(Icons.lock_outline, size: 20, color: AppTheme.textHint),
                    ),
                    validator: (v) => v != _passCtrl.text ? s.get('passwordsDontMatch') : null,
                  ),
                  const SizedBox(height: 28),
                  ...
                  Center(
                    child: GestureDetector(
                      ...
                      child: RichText(
                        text: TextSpan(
                          text: s.get('alreadyHaveAccount'),
                          style: const TextStyle(color: AppTheme.textLight, fontSize: 14),
                          children: [
                            TextSpan(text: s.get('signInLink'),
                                style: const TextStyle(color: AppTheme.heroStart, fontWeight: FontWeight.w700)),
                          ],
                        ),
                      ),
                    ),
  ...
  Widget _label(String t) => Text(t,
      style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AppTheme.textMedium));
}
